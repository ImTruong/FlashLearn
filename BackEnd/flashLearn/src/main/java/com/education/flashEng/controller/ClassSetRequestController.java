...
public class ClassSetRequestController {
    ...
    public ResponseEntity<?> acceptSetRequest(@PathVariable Long setRequestId) {
        ApiResponse<?> response = new ApiResponse<>(classSetRequestService.acceptSetRequest(setRequestId), "Accept Set Request Successfully");
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @PutMapping("/reject/{setRequestId}")
    public ResponseEntity<?> rejectSetRequest(@PathVariable Long setRequestId) {
        ApiResponse<?> response = new ApiResponse<>(classSetRequestService.rejectSetRequest(setRequestId), "Reject Set Request Successfully");
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
}
