...
public class UserController {
    ...
    public ResponseEntity<?> update(@Valid @RequestBody UpdateUserRequest updateUserRequest) {
        ApiResponse<String> response = new ApiResponse<String>(userService.update(updateUserRequest),"Update Account Successful");
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @PutMapping("/password")
    public ResponseEntity<?> updatePassword(@Valid @RequestBody UpdateUserPasswordRequest updateUserPasswordRequest) {
        ApiResponse<String> response = new ApiResponse<String>(userService.updatePassword(updateUserPasswordRequest),"Update Password Successful");
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
