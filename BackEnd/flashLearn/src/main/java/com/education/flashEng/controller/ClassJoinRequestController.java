...
public class ClassJoinRequestController {
    ...
    public ResponseEntity<?> createClassJoinRequest(@Valid @RequestBody ClassJoinRequestRequest classJoinRequestRequest) {
        try {
            ...
            ApiResponse<String> response = new ApiResponse<>(true, "Join Request Created Successfully.");
            return new ResponseEntity<>(response, HttpStatus.OK);
        } catch (Exception e) {
            ApiResponse<String> response = new ApiResponse<>(false, "Failed to create join request: " + e.getMessage());
    ...
    public ResponseEntity<?> acceptClassJoinRequest(@RequestParam @NotNull(message = "joinRequestId is required") Long requestId) {
        ApiResponse<String> response = new ApiResponse<>(classJoinRequestService.acceptClassJoinRequest(requestId), "Join Request Accepted Successfully.");
        return new ResponseEntity<>(response, HttpStatus.OK);
    }

    @DeleteMapping("/reject")
    public ResponseEntity<?> rejectClassJoinRequest(@RequestParam @NotNull(message = "joinRequestId is required") Long requestId) {
        ApiResponse<String> response = new ApiResponse<>(classJoinRequestService.rejectClassJoinRequest(requestId), "Request Rejected Successfully");
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> revokeClassJoinRequest(@RequestParam @NotNull(message = "requestId is required") Long classJoinRequestId) {
        ApiResponse<String> response = new ApiResponse<>(classJoinRequestService.revokeClassJoinRequest(classJoinRequestId), "Request Revoked Successfully");
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
}
